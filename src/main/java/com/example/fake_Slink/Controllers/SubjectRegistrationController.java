...
@RequestMapping("${api.prefix}/subject_registration")
public class SubjectRegistrationController {
    private final SubjectRegistrationService subjectRegistrationService;

    @PostMapping
    public ApiResponse<Boolean> subjectRegistration(
            @RequestBody @Valid List<CreateSubjectRegistrationRequest> list,
        ...
        try {
            Boolean status = subjectRegistrationService.createSubjectRegistration(list);
            return ApiResponse.<Boolean>builder()
